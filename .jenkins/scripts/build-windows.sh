#!/usr/bin/env bash
# Cross-compiles Windows x64 on the macOS agent (no cgo: WinFsp is loaded at runtime), then builds the NSIS
# installer. CGO_ENABLED=0 is passed as a Task variable only: the bindings step runs on the macOS host and
# needs cgo to see the darwin-only methods.
...
# Homebrew's makensis 3.12 bottle aborts with std::bad_alloc on every Unicode script. Probe it once and
# fall back to Debian's makensis in Docker, which builds the same project.nsi fine.
